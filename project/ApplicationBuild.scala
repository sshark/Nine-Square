...
//  lazy val root = Project(id = "root", base = file(".")) aggregate(common, gui, cmdline)
  lazy val root = Project("root", file(".")) aggregate(common, gui, web)

  lazy val web = Project("web", file("web")).enablePlugins(play.PlayScala).settings(
    libraryDependencies ++= Dependencies.authorization
  ) dependsOn(common)

  lazy val common = Project("common", file("common"),
    settings = standardSettings ++ Seq(libraryDependencies ++= Dependencies.common) ++ SbtOneJar.oneJarSettings)

  lazy val gui = Project("gui", file("gui"),
...
//      val slf4s = "com.weiglewilczek.slf4s" % "slf4s_2.9.1" % "1.0.7"
      ...
      val akka = "com.typesafe.akka" %% "akka-actor" % "2.3.10"
    ...
    val common = logging ++ testkit ++ Seq(akka, mysql, jdbc, anorm)
